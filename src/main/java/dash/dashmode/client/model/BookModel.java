...
public class BookModel<T extends Entity> extends EntityModel<T> {
    ...
    private final ModelPart leftBlock = (new ModelPart(64, 32, 0, 10)).addCuboid(0.0F, -4.0F, -0.99F, 5.0F, 8.0F, 1.0F);
    private final ModelPart rightBlock = (new ModelPart(64, 32, 12, 10)).addCuboid(0.0F, -4.0F, -0.01F, 5.0F, 8.0F, 1.0F);
    private final ModelPart leftPage = (new ModelPart(64, 32, 24, 10)).addCuboid(0.0F, -4.0F, 0.0F, 5.0F, 8.0F, 0.005F);
    private final ModelPart rightPage = (new ModelPart(64, 32, 24, 10)).addCuboid(0.0F, -4.0F, 0.0F, 5.0F, 8.0F, 0.005F);
    ...
    public void setAngles(T entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
        ...
        double angle = animationProgress % (Math.PI * 2);

        if (angle > Math.PI) {
            double subtract = (Math.PI - angle) * 2;
            angle += subtract;
        }

        angle /= Math.PI;

        setPageAngles(0, 0, 0, (float) angle);
    }
