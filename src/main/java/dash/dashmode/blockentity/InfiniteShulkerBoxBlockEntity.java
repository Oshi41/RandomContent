...
import dash.dashmode.block.InfiniteShulkerBoxBlock;
...
public class InfiniteShulkerBoxBlockEntity extends LockableContainerBlockEntity implements SidedInventory, Tickable {
    ...
    public InfiniteShulkerBoxBlockEntity() {
        super(DashBlockEntities.InfiniteShulker);

        int size = 27;
        inventory = DefaultedList.ofSize(size, CustomStackSize.createWithCustomStackSize(ItemStack.EMPTY, getMaxCountPerStack()));
        AVAILABLE_SLOTS = IntStream.range(0, size).toArray();
    }

    @Override
    protected Text getContainerName() {
        return new TranslatableText("block_entity_type.random_content.infinite_shulker_box");
    }

    @Override
    protected ScreenHandler createScreenHandler(int syncId, PlayerInventory playerInventory) {
        return new InfiniteShulkerBoxScreenHandler(syncId, playerInventory, this);
    }
    ...
    public boolean canInsert(int slot, ItemStack stack, @Nullable Direction dir) {
        boolean canInsert = !(Block.getBlockFromItem(stack.getItem()) instanceof InfiniteShulkerBoxBlock)
                && isValid(slot, stack);

        if (canInsert) {
            // changing stack size for hopper
    ...
    public ItemStack removeStack(int slot, int amount) {
        ...
        return CustomStackSize.withDefaultStackSize(itemStack);
    }

    @Override
    public ItemStack removeStack(int slot) {
        return CustomStackSize.withDefaultStackSize(Inventories.removeStack(inventory, slot));
    }

    @Override
    public void setStack(int slot, ItemStack stack) {
        inventory.set(slot, CustomStackSize.createWithCustomStackSize(stack, getMaxCountPerStack()));
    ...
    public CompoundTag toTag(CompoundTag tag) {
        return DashInventoryUtils.toTag(inventory, super.toTag(tag));
    }

    @Override
    public void fromTag(BlockState state, CompoundTag tag) {
        super.fromTag(state, tag);
        DashInventoryUtils.fromTag(tag, inventory, getMaxCountPerStack());
    }
